...
async function sendSlackAlert(mentions, video, creator) {
  ...
  const blocks = [
    {
      type: 'header',
      text: { type: 'plain_text', text: '🎯 Respawn Signal — Brand Alert', emoji: true },
    },
    {
      type: 'section',
  ...
  blocks.push({
    ...
    elements: [
      {
        type: 'mrkdwn',
        text: `Respawn Signal  ·  ${notable.length} notable mention${notable.length !== 1 ? 's' : ''}  ·  ${new Date().toUTCString()}`,
      },
    ],
  })
