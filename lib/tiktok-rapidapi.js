 ...
 *   1. Create a RapidAPI account and subscribe (free tier available)
...
// In-memory cache: username → secUid (lasts for the duration of the process)
const secUidCache = new Map()

/**
 * Resolves a TikTok username to its secUid via the User Info endpoint.
 * secUid is required by the User Posts endpoint.
 */
async function resolveSecUid(handle, apiKey) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`TikTok RapidAPI user lookup failed for @${handle} [${res.status}]: ${text.slice(0, 300)}`)
  }

  const json = await res.json()

  // Response shape varies: { data: { user } } or { userInfo: { user } }
  const secUid = json.data?.user?.secUid || json.data?.userInfo?.user?.secUid || json.userInfo?.user?.secUid
  if (!secUid) {
    throw new Error(`TikTok RapidAPI: could not resolve secUid for @${handle} — ${JSON.stringify(json).slice(0, 200)}`)
  }
...
async function fetchRecentVideos(creator, limit = 7) {
  ...
  // Step 1: resolve username → secUid
  const secUid = await resolveSecUid(handle, apiKey)
